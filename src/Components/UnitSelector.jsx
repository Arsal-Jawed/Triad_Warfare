...
export default function UnitSelector({ units, selectedUnit, onSelect, unitTypes }) {
  ...
  const [, drag] = useDrag({
    type: 'unit', // Defines that the draggable items are 'unit' type
    item: { unitImages, unitTypes }, // Item contains data to be passed to the drop target
  });
    ...
    <>
      {units.map((unit) => (
        <div
          ...
          className={`p-3 rounded-lg cursor-pointer transition ${selectedUnit === unit.id ? 'ring-2 ring-blue-500' : 'hover:bg-gray-800'} ${unitTypes[unit.type].color} text-white font-semibold relative overflow-hidden`}
        >
          <img
